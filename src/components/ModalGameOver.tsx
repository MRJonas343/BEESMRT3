  ...
  return (
    <div className={modalContainer}>
      <div className="bg-white w-[350px] rounded-xl p-7 h-auto absolute z-30 top-1/2 left-1/2 fixPosition lg:w-1/3">
        ...
        <hr />
        <p className="pt-3 text-5xl text-center font-Principal text-Pink1 tracking-wider">{message}</p>
        <img className="flex w-72 mx-auto pt-5" src={imageSrc} alt="Message Image" />
        <div className="flex justify-between pt-4">
          <button onClick={playAgain} className="bg-blue-700 text-white py-3 px-5 rounded-lg font-Principal text-xl">Play Again</button>

          <button onClick={showModalWin} className="bg-red-600 text-white py-3 px-10 rounded-lg font-Principal text-xl">Close</button>
        </div>
      </div>
    </div>
  )
