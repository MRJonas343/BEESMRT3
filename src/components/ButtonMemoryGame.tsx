...
const MemoryGameButtton: React.FC<MemoryGameButttonProps> = ({ card, flipped, chooseCard }) => {
  const cardHandleClick = () => chooseCard(card)

  const classCard = `card ${flipped ? 'matched' : ''} w-20 h-14 bg-gray-100 flex relative justify-center items-center cursor-pointer  md:w-24 md:h-20 xl:w-36 xl:h-32`
